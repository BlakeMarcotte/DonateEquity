...
import { DonationPathBuilderModal } from './DonationPathBuilderModal'
import { ReviewResultsModal } from './ReviewResultsModal'
import { DownloadTemplatesModal } from './DownloadTemplatesModal'
import { UploadCompletedFilesModal } from './UploadCompletedFilesModal'
import { QuizData } from '@/types/task'


interface PledgeTaskListProps {
...
export function PledgeTaskList({
  participantId,
  ...
  const { tasks: pledgeTasks, loading: pledgeLoading, completeTask: pledgeCompleteTask } = usePledgeTasks(participantId || null)

  // Use external tasks/handlers if provided, otherwise use pledge tasks
  const tasks = externalTasks || pledgeTasks
  const loading = externalLoading !== undefined ? externalLoading : pledgeLoading
  const completeTask = externalCompleteTask || pledgeCompleteTask
  ...
  const [showReviewResultsModal, setShowReviewResultsModal] = useState(false)
  const [currentReviewTask, setCurrentReviewTask] = useState<Task | null>(null)
  const [showDownloadTemplatesModal, setShowDownloadTemplatesModal] = useState(false)
  const [currentDownloadTemplatesTask, setCurrentDownloadTemplatesTask] = useState<Task | null>(null)
  const [showUploadCompletedFilesModal, setShowUploadCompletedFilesModal] = useState(false)
  const [currentUploadCompletedFilesTask, setCurrentUploadCompletedFilesTask] = useState<Task | null>(null)
  ...
  const handleQuizComplete = async (quizData: QuizData) => {
    if (currentQuizTask) {
      setCompletingTasks(prev => new Set(prev).add(currentQuizTask.id))
      try {
        await completeTask(currentQuizTask.id, quizData)
        ...
        setCompletingTasks(prev => {
          const newSet = new Set(prev)
          newSet.delete(currentQuizTask.id)
          return newSet
        })
      }
    }
  ...
  const handleReviewAccept = async () => {
    if (currentReviewTask) {
      setCompletingTasks(prev => new Set(prev).add(currentReviewTask.id))
      try {
        await completeTask(currentReviewTask.id)
        setShowReviewResultsModal(false)
        ...
        setCompletingTasks(prev => {
          const newSet = new Set(prev)
          newSet.delete(currentReviewTask.id)
          return newSet
        })
      }
    }
  ...
  const handleDownloadTemplatesComplete = async () => {
    if (currentDownloadTemplatesTask) {
      setCompletingTasks(prev => new Set(prev).add(currentDownloadTemplatesTask.id))
      try {
        await completeTask(currentDownloadTemplatesTask.id)
        setShowDownloadTemplatesModal(false)
        setCurrentDownloadTemplatesTask(null)
      } catch {
        // Error will be handled by the completion flow
        throw new Error('Failed to complete download templates')
      } finally {
        setCompletingTasks(prev => {
          const newSet = new Set(prev)
          newSet.delete(currentDownloadTemplatesTask.id)
          return newSet
        })
      }
    }
  ...
  const handleUploadCompletedFilesComplete = async () => {
    if (currentUploadCompletedFilesTask) {
      setCompletingTasks(prev => new Set(prev).add(currentUploadCompletedFilesTask.id))
      try {
        await completeTask(currentUploadCompletedFilesTask.id)
        setShowUploadCompletedFilesModal(false)
        setCurrentUploadCompletedFilesTask(null)
      } catch {
        // Error will be handled by the completion flow
        throw new Error('Failed to complete upload completed files')
      } finally {
        setCompletingTasks(prev => {
          const newSet = new Set(prev)
          newSet.delete(currentUploadCompletedFilesTask.id)
          return newSet
        })
      }
    }
  ...
  const handleCompleteTask = async (taskId: string) => {
    ...
    // Check if this is a review results task
    if (task?.type === 'review_results') {
      setCurrentReviewTask(task)
      setShowReviewResultsModal(true)
      return
    }

    // Check if this is a download templates task (can be reopened even when completed)
    if (task?.type === 'download_templates') {
      setCurrentDownloadTemplatesTask(task)
      setShowDownloadTemplatesModal(true)
      return
    }

    // Check if this is an upload completed files task
    if (task?.type === 'upload_completed_files') {
      setCurrentUploadCompletedFilesTask(task)
      setShowUploadCompletedFilesModal(true)
      return
    }
  ...
  const handleInvitationSuccess = () => {
    ...
    // The usePledgeTasks hook should automatically refresh and show the task as completed
  }
    ...
    try {
      ...
      const envelopeResponse = await fetch('/api/docusign/create-envelope', {
        ...
        body: JSON.stringify({
          ...
          emailSubject: 'Please sign the General NDA for your pledge'
        })
      })
  ...
  const canCompleteTask = (task: Task) => {
    // Allow reopening completed download_templates tasks to view signers
    if (task.type === 'download_templates' && task.status === 'completed') {
      return true
    }
  ...
  // Tasks should always exist since they're created automatically with pledges
    ...
    return (
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-md border border-gray-100/50 p-6">
        <div className="text-center text-gray-500">
          ...
          <p className="text-sm text-gray-600">Your pledge tasks are being loaded.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-bold bg-gradient-to-r from-[#005A7D] to-[#004560] bg-clip-text text-transparent">
          {showAllTasks ? 'Pledge Workflow Tasks' : 'Your Impact Tasks'}
        </h2>
        <div className="flex items-center space-x-3">
          <div className="px-3 py-1 bg-gradient-to-r from-[#5BC5F2]/20 to-[#005A7D]/10 rounded-full border border-[#5BC5F2]/30">
            <span className="text-xs font-semibold text-[#005A7D]">
              {sortedTasks.filter(t => t.status === 'completed').length} of {sortedTasks.length} completed
            </span>
          </div>
          {/* Refresh button - Pledge 1% Branded */}
          <Button
            onClick={handleRefreshTasks}
            disabled={refreshing}
            variant="outline"
            size="sm"
            className="text-[#005A7D] border-[#5BC5F2]/40 hover:bg-[#5BC5F2]/10 rounded-xl transition-all duration-200"
          >
            {refreshing ? (
              <>
                <div className="animate-spin -ml-1 mr-2 h-4 w-4 border-2 border-[#005A7D] border-t-transparent rounded-full" />
                Refreshing...
              </>
            ) : (
              <>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3 flex-1">
                  <div className="flex flex-col items-center">
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#005A7D] to-[#004560] flex items-center justify-center text-xs font-bold text-white mb-2 shadow-md">
                      {index + 1}
                    </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2 mb-1">
                      <h4 className="text-base font-bold text-[#005A7D]">
                        {task.title}
                      </h4>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium shadow-sm ${
                        task.assignedRole === 'donor' ? 'bg-gradient-to-r from-[#5BC5F2]/20 to-[#5BC5F2]/30 text-[#005A7D] border border-[#5BC5F2]/30' :
                  ...
                  {canCompleteTask(task) ? (
                    <div className="flex flex-col gap-1.5">
                      <Button
                        ...
                        className="bg-gradient-to-r from-[#005A7D] to-[#004560] hover:from-[#004560] hover:to-[#003347] text-white shadow-md hover:shadow-lg transition-all duration-200 rounded-lg px-4 py-2 font-semibold"
                      >
                        ...
                        ) : task.type === 'download_templates' && task.status === 'completed' ? (
                          <>
                        ...
                        ) : task.type === 'upload_completed_files' ? (
                          <>
                            <Upload className="h-4 w-4 mr-2" />
                            Upload Signed Files
                          </>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-gray-100">
            ...
            <Button
              ...
              className="bg-gradient-to-r from-[#005A7D] to-[#004560] hover:from-[#004560] hover:to-[#003347] text-white shadow-lg hover:shadow-xl transition-all duration-200 rounded-xl px-6 py-3 font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Upload className="h-4 w-4 mr-2" />
              Upload & Complete
            </Button>
      ...
      {/* Donation Path Builder Quiz Modal */}
      <DonationPathBuilderModal
      ...
      <ReviewResultsModal
        ...
        quizData={(() => {
          // Find the completed quiz task to get the quiz data
          const quizTask = tasks.find(t => t.type === 'quiz' && t.status === 'completed')
          return (quizTask?.completionData as QuizData) || null
        })()}
        campaignTitle={campaignTitle || 'this campaign'}
        donorName={donorName || 'Anonymous Donor'}
      />
      ...
      <DownloadTemplatesModal
        isOpen={showDownloadTemplatesModal}
        onClose={() => {
          setShowDownloadTemplatesModal(false)
          setCurrentDownloadTemplatesTask(null)
        }}
        onComplete={handleDownloadTemplatesComplete}
        quizData={(() => {
          // Find the completed quiz task to get the quiz data
          const quizTask = tasks.find(t => t.type === 'quiz' && t.status === 'completed')
          return (quizTask?.completionData as QuizData) || null
        })()}
        viewOnly={currentDownloadTemplatesTask?.status === 'completed'}
      />

      {/* Upload Completed Files Modal */}
      <UploadCompletedFilesModal
        isOpen={showUploadCompletedFilesModal}
        onClose={() => {
          setShowUploadCompletedFilesModal(false)
          setCurrentUploadCompletedFilesTask(null)
        }}
        onComplete={handleUploadCompletedFilesComplete}
        quizData={(() => {
          // Find the completed quiz task to get the quiz data
          const quizTask = tasks.find(t => t.type === 'quiz' && t.status === 'completed')
          return (quizTask?.completionData as QuizData) || null
