    ...
    <>
      ...
      <div className="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg border-r border-gray-200 hidden lg:flex lg:flex-col">
        ...
        <div className="flex items-center px-6 py-4 border-b border-gray-200">
          <Link href={userRole === 'donor' ? '/my-donations' : userRole === 'appraiser' ? '/appraiser' : '/tasks'} className="flex items-center space-x-3">
        ...
        {(userRole === 'donor' || userRole === 'appraiser') && campaign && (
          <div className="px-4 py-4 border-b border-gray-200">
            <Link
              href={userRole === 'donor' ? '/my-donations' : '/my-campaign'}
              className={`flex flex-col space-y-1 px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 ${
                (userRole === 'donor' ? pathname === '/my-donations' : pathname === '/my-campaign')
                  ? 'bg-blue-50 text-blue-600 border-r-2 border-blue-600'
                  : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
              }`}
            >
              <div className="text-xs font-medium text-blue-600 uppercase tracking-wide">
                {userRole === 'donor' ? 'Your Donation' : 'Your Campaign'}
              </div>
              <div className={`font-semibold truncate ${
                (userRole === 'donor' ? pathname === '/my-donations' : pathname === '/my-campaign') ? 'text-blue-600' : 'text-gray-900'
              }`}>
                {campaign.title}
              </div>
              {campaign.organizationName && (
                <div className={`text-xs truncate ${
                  (userRole === 'donor' ? pathname === '/my-donations' : pathname === '/my-campaign') ? 'text-blue-500' : 'text-gray-500'
                }`}>
                  {campaign.organizationName}
                </div>
              )}
      ...
      <div className="lg:hidden">
        ...
        <div className="bg-white shadow-sm border-b border-gray-200 px-4 py-3">
          <div className="flex items-center justify-between">
            <Link href={userRole === 'donor' ? '/my-donations' : userRole === 'appraiser' ? '/appraiser' : '/tasks'} className="flex items-center space-x-2">
              <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center">
                <Heart className="w-5 h-5 text-white" />
              </div>
