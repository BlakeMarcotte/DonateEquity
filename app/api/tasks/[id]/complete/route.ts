...
export async function POST(
  ...
  try {
    ...
    if (taskData?.participantId) {
      await updateDependentTasksForParticipant(taskId, taskData.participantId)
    } else if (taskData?.donationId) {
...
// Helper function to update dependent tasks for participant-based tasks
async function updateDependentTasksForParticipant(completedTaskId: string, participantId: string) {
  try {
    console.log(`Starting dependency update for completed task: ${completedTaskId} (participant: ${participantId})`)
    ...
    console.log(`Found ${allTasks.length} total tasks for participant ${participantId}`)
...
// Helper function to update dependent tasks when a task is completed (legacy)
